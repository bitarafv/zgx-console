...
Public allowlist: `GET /demo/*`, `GET /api/resources`, `/api/workloads`, `/api/policy`, and `/api/capability`.

Keep a Bypass policy for everyone on `/demo/*` above protected rules. Create one Cloudflare Access application for `/admin*` (including booking form, live events, and `/admin/siva` descendants), and separate applications only for machine-facing `/api/admin`, `/api/transitions`, and `/api/workloads/*/stop`, allowing only `bitarafv@gmail.com`. The application independently verifies the Access email before enabling mutations. Without the Access identity, `/api/admin/*` returns 403 even if the edge policy is missing.
