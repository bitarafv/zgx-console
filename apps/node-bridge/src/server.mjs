...
export function allowedMutation(path){return path==="/api/transitions"||new RegExp(`^/api/workloads/${safeId}/stop$`).test(path)}
export function allowedAdminRead(path){return path==="/"||new RegExp(`^/launch/${safeId}$`).test(path)||path==="/api/transitions"||new RegExp(`^/api/transitions/${safeId}(?:/(?:events|report))?$`).test(path)}
export function externalize(path,value){
 const clean=item=>({...item,browser_url:typeof item.external_browser_url==="string"&&item.external_browser_url.startsWith("https://")?item.external_browser_url:typeof item.browser_url==="string"&&item.browser_url.startsWith("https://")?item.browser_url:null});
...
async function proxy(req,res,path){
 ...
 try{
  ...
  const upstreamReq=http.request({hostname:sivaOrigin.hostname,port:sivaOrigin.port||80,path,method:req.method,headers:{host:publicHostname,...(body?{"content-type":"application/json","content-length":String(body.length)}:{})}},upstream=>{
