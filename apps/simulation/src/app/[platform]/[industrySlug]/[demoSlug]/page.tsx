...
export default async function PlatformDemoPage({ params }: { params: Promise<{ platform: string; industrySlug: string; demoSlug: string }> }) {
  ...
  return <div className="shell pt-10"><PlatformSelectionSync platform={platform}/><Link href={`//`} className="muted inline-flex items-center gap-2 text-sm font-bold"><ArrowLeft size={16}/> {demo.industry}</Link>
